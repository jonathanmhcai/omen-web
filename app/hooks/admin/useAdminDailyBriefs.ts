...
interface UseAdminDailyBriefsOptions {
  limit?: number;
...
export function useAdminDailyBriefs({
  limit = 15,
  sorting,
  filters,
  search,
}: UseAdminDailyBriefsOptions = {}) {
  const [briefs, setBriefs] = useState<AdminDailyBrief[]>([]);
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/admin/daily-briefs?${params}`, {

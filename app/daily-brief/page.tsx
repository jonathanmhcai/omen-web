...
/**
 * The daily brief. `?user=<handleOr0x>` renders that trader's brief: the
 * server returns the email body pre-rendered by the SAME react-email
 ...
 * behind a skeleton.
 *
 * Briefs lived at `/` between 2026-08-11 and 2026-08-20, so `/?user=` is in
 * the wild in sent emails; next.config.ts redirects that shape back here.
 */
...
export async function generateMetadata({
  ...
  // The bare lookup landing is indexable and inherits the root layout's OG.
  // Per-trader briefs are shareable but not indexable (link-unfurl bots
  // still read the tags).
  if (!user) {
    return {
      title: "Daily brief",
      description: "News on your Polymarket positions, every morning.",
    };
  }
